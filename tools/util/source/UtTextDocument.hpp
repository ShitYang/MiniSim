...
public:
   ...
   // TEXT_DOC: 将文件打开，并返回stream
   bool ReadFile(const UtPath& aPath);
   // TEXT_DOC: 将stream中的内容读到GapBuffer中
...
protected:
   ...
   void InsertLineData(size_t aPosition, size_t aCount)
   {
      ...
      for (size_t i = 0; i < aCount; ++i)
      {
         ...
         if (c == '\n')
         {
            ...
            charOffset         = i + 1;
            // TEXT_DOC: 在currentLine插入一行，
            // TEXT_DOC: 这一行的开头起始位置不变，之后的行要+insertChars
   ...
   void SplitLine(size_t aLineNumber, size_t aColumn)
   {
      ...
      {
         // TEXT_DOC: 为什么先MoveLineBreak(xxx+1)?
         // TEXT_DOC: 因为不移动，直接插入，新行默认会有一个mUpdateChars偏移量
         // TEXT_DOC: 在后来的MoveLineBreak后，可能会错误地加这个偏移量
         MoveLineBreak(aLineNumber + 1);
         LineData l;
         // TEXT_DOC: aColcum是插入位置到插后结尾，一共的字符数
         // TEXT_DOC: l.mPosition是下一行行头位置
   ...
   // FUCK (Lazy)mLastUpdateLine记录正确位置的最后一行，其他行的字符位置必须加上mUpdateChars，这样避免每次都更新
