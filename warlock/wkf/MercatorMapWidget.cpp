...
MercatorMapWidget::MercatorMapWidget(QWidget *parent)
    : QOpenGLWidget(parent), m_program(nullptr),
      ...
      ,
      m_dragging(false), m_gridDensity(10), m_bgColor(255, 255, 255, 255),
      m_gridColor(0, 0, 0, 255), m_equatorColor(255, 100, 100) // 红色赤道
      ,
  ...
  if (MainWindow *window = dynamic_cast<MainWindow *>(parent)) {
    connect(window, &MainWindow::SignalUpdatePlatformMap, this,
            &MercatorMapWidget::UpdatePlatformMap);
  }

  connect(&mUpdateTimer, &QTimer::timeout, this, [this]() { this->update(); });
...
double MercatorMapWidget::mercatorYToLat(double y) const {
  ...
  double lat_rad =
      2.0 * std::atan(std::exp(y_meters / EARTH_RADIUS)) - PI / 2.0;
...
void MercatorMapWidget::updateGridData() {
  ...
  if (m_gridVao.isCreated()) {
    ...
    // 上传到VBO
    m_gridVbo.bind();
    m_gridVbo.allocate(vertexData.constData(),
...
void MercatorMapWidget::paintGL() {
  ...
  if (m_gridVao.isCreated()) {
    ...
    glDrawArrays(GL_LINES, 0,
                 (m_longitudeLines.size() + m_latitudeLines.size()) * 2);
    ++m_drawCallCount;
    m_gridVao.release();
  }
  ...
  // Draw overlay: labels + stats
  {
    ...
    for (const auto *f : visibleFeatures) {
      ...
      if (screenPos.x() < 0 || screenPos.x() > width() || screenPos.y() < 0 ||
          screenPos.y() > height())
        continue;
    ...
    int lineY = boxY + 18;
    painter.drawText(boxX + 8, lineY,
                     QString("FPS: %1").arg(m_currentFPS, 0, 'f', 1));
    lineY += 18;
    painter.drawText(boxX + 8, lineY,
                     QString("Draw Calls: %1").arg(m_currentDrawCalls));
    lineY += 18;
    painter.drawText(boxX + 8, lineY,
                     QString("VRAM Est: %1 MB").arg(m_estimatedVRAM_MB));
    lineY += 18;
    painter.drawText(
        boxX + 8, lineY,
        QString("Vector Features: %1").arg(visibleFeatures.size()));
  }
}
...
QMatrix4x4 MercatorMapWidget::getProjectionMatrix() const {
  ...
  projection.ortho(-worldWidth / 2, worldWidth / 2, -worldHeight / 2,
...
void MercatorMapWidget::zoomIn() { setZoomLevel(m_zoomLevel + 0.5); }

void MercatorMapWidget::zoomOut() { setZoomLevel(m_zoomLevel - 0.5); }
...
void MercatorMapWidget::PeparePlatformSprites() {
  ...
  for (auto &sprite : resources.mSpriteMap) {
    ...
    sprite.second->mTexturePtr =
        std::make_unique<QOpenGLTexture>(img.mirrored());
  ...
  // 顶点: {x, y, u, v}
  float vertices[] = {-size, -size, 0.0f,  1.0f, size, -size,
                      1.0f,  1.0f,  size,  size, 1.0f, 0.0f,

                      -size, -size, 0.0f,  1.0f, size, size,
                      1.0f,  0.0f,  -size, size, 0.0f, 0.0f};
  ...
  m_platformProgram->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                             VERTEX_SHADER_SRC);
  m_platformProgram->addShaderFromSourceCode(
...
int MercatorMapWidget::RenaderPlatforms() {
  ...
  for (const auto &item : m_platformMap) {
    double lat = item.second.mLocationWCS.at(0),
           lon = item.second.mLocationWCS.at(1);
...
void appendQuad(std::vector<float> &out, float x0, float y0, // 左上 (p1)
...
void MercatorMapWidget::PrepareRenderTile() {
  TiffLoader &tileLoader = wkfEnv.GetResourceManager().GetTileLoader();
  std::vector<Bound> &&grids = tileLoader.GetCurrentViewportGrids(
      m_viewCenter.x(), m_viewCenter.y(), m_zoomLevel);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * grids.size() * 6 * 4,
               vertcies.data(), GL_STATIC_DRAW);
  ...
  m_tileProgram->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                         VERTEX_SHADER_SRC);
  m_tileProgram->addShaderFromSourceCode(
      QOpenGLShader::Fragment, "#version 330 core\n"
                               "in vec2 TexCoord;"
                               "out vec4 FragColor;"
                               "uniform sampler2D ourTexture;"
                               "void main() {"
...
int MercatorMapWidget::RenderTiles() {
  ...
  // Get Tiles
  std::vector<Tiff> tiffs = tileLoader.GetCurrentViewportTiffs(
      m_viewCenter.x(), m_viewCenter.y(), m_zoomLevel);
...
int MercatorMapWidget::RenderVectorFeatures() {
  ...
  m_vectorRenderer->PrepareRender(
      visibleFeatures, m_zoomLevel,
      [this](double lon, double lat) { return projectLonLat(lon, lat); });
