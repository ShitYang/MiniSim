...
inline QDataStream& operator<<(QDataStream& aStream, const Filter& aArg)
{
   aStream << aArg.type;
   aStream << aArg.value;
   aStream << QVariant{aArg.isFiltered};
   aStream << QVariant{aArg.isInclusive};
   return aStream;
}

inline QDataStream& operator>>(QDataStream& aStream, Filter& aArg)
{
   QVariant typeVar, valueVar;

   aStream >> aArg.type >> aArg.value >> typeVar >> valueVar;
   
   aArg.type = typeVar.toBool();
   aArg.value = valueVar.toBool();
