                                   ...
                                   const bool                       aScriptLocal)
{
   ...
   if (aScript.script.context == warlock::script::GLOBAL)
   {
      ...
      qWarning() << "Script Error: " << "No platform context for script " << QString::fromStdString(aScript.script.name);
      return false;
   }
   ...
   if (contextPlatform == nullptr)
   {
      ...
      qWarning() << "Script Error: " << QString("Platform (%1) does not exist").arg(QString::fromStdString(aScript.platformName));
      return false;
   }
            ...
            aArgument.type == "Array<WsfComm>")
   {
      ...
      for (const auto& partName : aArgument.data.value<QVector<QVariant>>())
      {
         ...
         if (part == nullptr)
         {
            ...
            qWarning() << "Script Error: " << QString("Part (%1) does not exist on platform (%2)").arg(partName.toString())
   ...
   else
   {
      ...
      qWarning() <<  "Script Error: " << QString("Part (%1) is of invalid type (%2)").arg(aArgument.data.toString(), QString::fromStdString(aArgument.type));
      return false;
   }
                                     ...
                                     bool&                            aOk)
{
   ...
   for (const auto& iter : aScript.args)
   {
      ...
      else if (iter.type == "WsfGeoPoint")
      {
         ...
         {
            qWarning() << "Script Error: " <<  "Invalid GeoPoint format.";
            aOk = false;
         }
      }
      else if (iter.type == "WsfPlatform")
      {
         ...
         if (!platform)
         {
            qWarning() << "Script Error: " << "Platform (" + iter.data.toString() + ") not found.";
            aOk = false;
         }
      ...
      else if (iter.type == "WsfTrack")
      {
         ...
         //    else
         //    {
         //       qWarning() << "Script Error" << "Track (" + trackId + ") not found.";
         //       aOk = false;
         //    }
         // }
         // else
         // {
         //    qWarning() << "Script Error" << QString("Platform (%1) not found.").arg(platformName.c_str());
         //    aOk = false;
         // }
      }
      ...
      else if (iter.type == "Array<WsfPlatform>")
      {
         ...
         for (const auto& platformName : iter.data.value<QVector<QVariant>>())
         {
            ...
            if (!platform)
            {
               qWarning() << "Script Error: " << "Platform (" + platformName.toString() + ") not found.";
               aOk = false;
            }
            else if (!aScriptLocal)
            {
      ...
      {
         qWarning() << "Script Error: " << "Unknown argument Type " + QString::fromStdString(iter.type);
         aOk = false;
      }
   }
   return retVal;
...
void warlock::ScriptExecuteCommand::Process(WsfSimulation& aSimulation)
{
   ...
   else // context == PLATFORM
   {
      ...
      {
         qWarning() << "Script Error: " << "Platform (" + QString::fromStdString(mScript.platformName) + ") not found.";
      }
   }
}
