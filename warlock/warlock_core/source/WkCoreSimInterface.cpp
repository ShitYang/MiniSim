...
void warlock::CoreSimInterface::PlatformAdded(double aSimTime, WsfPlatform& aPlatform)
{
   ...
   case (WSF_SPATIAL_DOMAIN_AIR):
      pp.mSpatialDomain = SpatialDomain::SPATIAL_DOMAIN_AIR;
      break;
   case (WSF_SPATIAL_DOMAIN_LAND):
      pp.mSpatialDomain = SpatialDomain::SPATIAL_DOMAIN_LAND;
      break;
   case (WSF_SPATIAL_DOMAIN_SPACE):
      pp.mSpatialDomain = SpatialDomain::SPATIAL_DOMAIN_SPACE;
      break;
   case (WSF_SPATIAL_DOMAIN_SURFACE):
      pp.mSpatialDomain = SpatialDomain::SPATIAL_DOMAIN_SURFACE;
      break;
   case (WSF_SPATIAL_DOMAIN_SUBSURFACE):
      pp.mSpatialDomain = SpatialDomain::SPATIAL_DOMAIN_SUBSURFACE;
      break;
   default:
      break;
   }
...
void warlock::CoreSimInterface::WallClockRead(const WsfSimulation& aSimulation)
{
   ...
   for (size_t i = 0; i < aSimulation.GetPlatformCount(); ++i)
   {
      ...
      auto lla = platformPtr->GetLocationLLA();
      auto ori = platformPtr->GetOrientationNED();
      ...
      proxy.mIcon = platformPtr->GetIcon();
      proxy.mName = platformPtr->GetName();
      proxy.mUpdateTime = aSimulation.GetSimTime();
      
      proxy.mLocationWCS[0] = lla.mLat, 
      proxy.mLocationWCS[1] =  lla.mLon,
      proxy.mLocationWCS[2] =  lla.mAlt;
      
      proxy.mOrientationWCS[0] = ori.mPsi, 
      proxy.mOrientationWCS[1] =  ori.mTheta, 
      proxy.mOrientationWCS[2] =  ori.mPhi;
